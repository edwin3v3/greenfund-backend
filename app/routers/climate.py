...
async def _fetch_weather_data(latitude: float, longitude: float, daily_params: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"ERROR: Open-Meteo API returned status {e.response.status_code}: {e.response.text}")
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY,
                            detail=f"Weather service returned an error.")
        ...
        print(
            f"ERROR: An unexpected error occurred while fetching weather data: {e}")
...
async def get_weather_forecast_and_recommendations(  # Renamed function for clarity
    ...
    if farm.owner_id != current_user.id:  # Ensure ownership check
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Not authorized to view this farm")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"An error occurred generating recommendations: {e}")
