...
async def _fetch_weather_data(latitude: float, longitude: float, daily_params: str):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            print(
                f"WARN: Attempt {attempt + 1}/{max_retries + 1} failed to fetch weather: {e}")
            if attempt == max_retries:
                if isinstance(e, httpx.HTTPStatusError):
                    print(
                        f"ERROR: Open-Meteo API returned status {e.response.status_code} after retries: {e.response.text}")
                    raise HTTPException(
                        status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Weather service error after retries.")
                else:
                    print(
                        f"ERROR: Could not connect to Open-Meteo API after {max_retries + 1} attempts: {e}")
                    raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                                        detail="Could not connect to the weather service after retries.")
            await asyncio.sleep(base_delay * (2 ** attempt))
        except Exception as e:
            print(
                f"ERROR: An unexpected error occurred during weather fetch: {e}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                detail="An unexpected error occurred fetching weather data.")
    raise HTTPException(
        status_code=500, detail="Weather fetch failed unexpectedly after retries.")
...
async def get_pest_disease_alerts(
    ...
    try:
        ...
        print(
            f"ERROR: Unexpected error calling _fetch_weather_data for alerts: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to retrieve weather data for alerts.")

    pest_risk_assessment = assess_pest_disease_risks(
        forecast_data, farm.current_crop)
    ...
    try:
        ...
        raise HTTPException(status_code=e.status_code or 500,
                            detail=f"AI pest analysis failed: {getattr(e, 'message', str(e))}")
    except Exception as e:
        print(
            f"ERROR: Unexpected error during AI pest analysis refinement: {e}")
        raise HTTPException(
            status_code=500, detail=f"AI pest analysis refinement failed: {e}")
...
async def get_carbon_sequestration_guidance(farm_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    activities = db.exec(select(FarmActivity).where(
        FarmActivity.farm_id == farm_id).order_by(desc(FarmActivity.date)).limit(10)).all()
    carbon_trend_assessment = assess_carbon_trend(activities)

    try:
        client = get_openai_client()
        activity_summary = ", ".join(
            list(set([a.activity_type for a in activities]))) or "no activities logged"
        prompt = f"""
        ...
        raise HTTPException(status_code=e.status_code or 500,
                            detail=f"AI carbon analysis failed: {getattr(e, 'message', str(e))}")
    except Exception as e:
        print(
            f"ERROR: Unexpected error during AI carbon analysis refinement: {e}")
        raise HTTPException(
            status_code=500, detail=f"AI carbon analysis refinement failed: {e}")
...
async def get_water_management_advice(farm_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        print(
            f"ERROR: Unexpected error calling _fetch_weather_data for water advice: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to retrieve weather data for water advice.")
    ...
    try:
        ...
        raise HTTPException(status_code=e.status_code or 500,
                            detail=f"AI water analysis failed: {getattr(e, 'message', str(e))}")
    except Exception as e:
        print(
            f"ERROR: Unexpected error during AI water analysis refinement: {e}")
        raise HTTPException(
            status_code=500, detail=f"AI water analysis refinement failed: {e}")
